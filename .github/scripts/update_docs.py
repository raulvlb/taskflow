...
def _build_prompt(current_docs: str, diff: str, source_snapshot: str) -> str:
    return (
        ...
        "- CROSS-REFERENCE every existing RF against index.html and app.js (the ground truth).\n"
        "  A feature EXISTS only if it has BOTH a UI element in index.html AND logic in app.js.\n"
        "  CSS alone does NOT prove a feature exists — ignore CSS-only evidence.\n"
        "  If no matching HTML element and JS function are found, REMOVE that RF.\n"
